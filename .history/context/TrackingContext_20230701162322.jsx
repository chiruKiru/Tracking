    ...
    const createFund = async (items) => {
        ...
        const TotalValues = async () => {
            try {
                const provider = new ethers.providers.JsonRpcProvider();
                const contract = fetchContract(provider);
                const Tranc = await contract.Totals();
                const values = Tranc.map((Transaction) => ({
                    CentralApp: Transaction.CentralApproved.toNumber(),
                    CentralRejected: Transaction.CentralRejected.toNumber(),
                    TotalRequest: Transaction.TotalRequest.toNumber(),
                    ToatlRejected: Transaction.ToatlRejected.toNumber(),
                    CentralRequest: Transaction.CentralRequest.toNumber(),
                    CentralTotal: Transaction.CentralTotal.toNumber()
                }));
                console.log(values.CentralApp);
                return values;
            } catch (error) {
                console.log("Error",error);
