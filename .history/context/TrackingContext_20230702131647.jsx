    ...
    const createFund = async (items) => {
        ...
        const TotalValues = async () => {
            try {
                const provider = new ethers.providers.JsonRpcProvider();
                const contract = fetchContract(provider);
                const Transaction = await contract.Totals().call();
                const CentralApproved = Transaction[0].toNumber();
                const CentralRejected = Transaction[1].toNumber();
                const TotalRequest = Transaction[2].toNumber();
                const ToatlRejected = Transaction[3].toNumber();
                const CentralRequest = Transaction[4].toNumber();
                const CentralTotal = Transaction[5].toNumber();
