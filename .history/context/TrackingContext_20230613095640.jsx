...
export const TransactionProvider =({ children })=> {
  ...
  const state = '0xf39Fd6e51aad88F6F4ce6aB8827279cffFb92266'
    const createFund = async (items) => {
        console.log(items);
        const {name,sender,ReqDate,from,amount,Reason,image,feedback,To,file,Id_proof} = items;
        var payer;
        To != central ? payer = To : payer = central
            try {
                ...
                const createReq = await contract.createFund(
                    sender,
                    name,
                    ReqDate,from,amount,Reason,image,feedback,payer,file,Id_proof
                    );
