...
export default ({getModel, setGetModel, getFund,allShipmentsdata,startModel,setStartModel,startFund}) => {
    ...
    <div className="fixed inset-0 z-10 overflow-y-auto">
      <div className="fixed inset-0 w-full h-full bg-black opacity-40" onClick={()=> setGetModel(false)}>
        ...
        <div className="flex items-center min-h-screen px-4 py-8">
          ...
          <button className='block w-full mt-3 py-4 font-medium text-sm text-center text-white bg-red-600
          hover:bg-indigo-500 active:bg-indigo-700 rounded-lg ring-offset-2 ring-red-600 focus:ring-2'>
            REJECT
          </button>
          </div>
        </div>
      ...
      {<tbody className="text-gray-600 divide-y">
        {allShipmentsdata?.map((shipment, idx) => (
          ...
          <tr key={idx}>
            ...
            rounded-full md:inline-flex" onClick={() =>{setGetModel(true); setIndex(idx); getShipmentData(); setGetProduct(shipment.receiver)}}>
             View
            </button>
