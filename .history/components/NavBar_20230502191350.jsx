...
import { Nav1,Nav2,Nav3 } from "./com";


export default() => {
  const {connectWallet,currentUser}  = useContext(TransactionContext)
    ...
    <nav className={`bg-white pb-5 md:text-sm ${state ? "shadow-lg rounded-xl border mx-2 mt-2 md:shadow-none md:border-none md:mx-2 md:mt-0 ": " "}`}>
      ...
      <div className="flex items-center justify-between py-5 md:block">
        ...
        <div className="md:hidden">
          <button className="menu-btn text-grey-500 hover:text-gray-800" onClick={()=>setState(!state)}>
            {state ? <Nav1/> : <Nav2/>}
          </button>
        </div>
      </div>
        ...
        <ul className="justify-center items-center space-y-6 md:flex md:space-x-6 md:space-y-0">
          {navigation.map((item,idx)=>{
            ...
            <div className="flex-1 gap-x-6 items-center justify-end mt-6 space-y-6 md:flex md:space-y-0 md:mt-0">
              {currentUser ? (
                <p className="flex item-center justify-center gap-x-1 py-2 px-4 text-white font-medium bg-grey-800 hover:bg-gray-700 active:bg-gray-900
