...
export default ({
  ...
  const convertEtherToRupees = async (amount) => {
    try {
      const response = await axios.get('https://api.example.com/eth-to-inr'); // Replace with the actual API endpoint
      const rate = response.data.rate; // Assuming the API response provides the exchange rate
      const rupees = (parseFloat(amount) * parseFloat(rate)).toFixed(2);
      console.log(rupees)
    } catch (error) {
  ...
  const onSubmitHandler = async (event) => {
    ...
    event.target.name == "Id_proof"? setRequest({ ...request, Id_proof: result.path}):
    event.target.name == "image"? setRequest({ ...request, image: result.path}):
  ...
  return (
          ...
          <form id='Data'onSubmit={(e)=> {e.preventDefault()}}>
            ...
            <input id="q" type="text" name="q" className={mass} value={request.ReqDate} onChange={()=>""}/>
                ...
                <div className="relative mt-5">
              <input
                type='text' id='amo' placeholder="amount in INR" className={mass} maxLength={7}
                ...
                <div className="relative mt-5">
                <select name="request" id="req" className={`${request.To !='0' ? mass : err}`} onChange={(e)=>{setRequest({ ...request, To: e.target.value})}}>
            ...
            <p align='left'> Upload your Id proof </p>
              {ipfs && ( 
                    <input type="file" className={mass} name="Id_proof" onChange={onSubmitHandler} disabled={request.Id_proof != "" ? true : false}/>
              ) }
                {request.Id_proof != '' ?(
              ...
              {ipfs && ( 
                    <input type="file" className={mass} name="image" onChange={onSubmitHandler}  disabled={request.image != "" ? true : false}/>
              )}
                {request.image != '' ?(
          ...
          <p align='left'> Upload a Govt verified file </p>
              {ipfs && ( 
                    <input type="file" className={mass} name="Doc" onChange={onSubmitHandler}  disabled={request.file != "" ? true : false}/>
              )}
                {request.file != '' ?(
          ...
          <div className="relative mt-5">
          <button onClick={convertEtherToRupees,console.log(xmount)} className='bg-indigo-900 text-white hover:bg-blue-400 font-bold py-2 px-4 mt-3 rounded items-center'>
            Request Funds
          </button>
          </div>
          </form> 
