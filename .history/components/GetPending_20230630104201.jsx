...
export default ({getModel, setGetModel, getFundMain,alldata,startFund,RejectedFund}) => {
  ...
  const reject = () => {
    Swal.fire({
      ...
      if (result.value) {
        RejectedFund(rejectfeedback,result)
      }
  });
  }
  ...
  /*const reject = () => {
    Swal.fire({
      ...
      if (result.value) {
        const ans  = RejectedFund(rejectfeedback,result.value)
        if(ans){
          window.location.reload(true)
        }
      }
      else{
        Swal.fire('You need to give feedback!')
      }
  });
  }
  */

  const stateApprove = async() => {
    const val = await startFund(getProduct);
    showAlert(val)
    setGetModel(!val)
  };
    ...
    <div className="fixed inset-0 z-10 overflow-y-auto">
      <div className="fixed inset-0 w-full h-full bg-black opacity-40" onClick={()=> setGetModel(false)}>
        ...
        <div className="flex items-center min-h-screen px-4 py-8">
          ...
          <button onClick={()=> stateApprove()} className='block w-full mt-3 py-4 font-medium text-sm text-center text-white bg-indigo-600
    ...
    <table className="w-full table-auto text-sm text-left">
      ...
      {<tbody className="text-gray-600 divide-y">
        {alldata?.map((fund, idx) => (
          fund.status == 0 && fund.from == 'Citizen' && fund.To != 'Central' ?(
              <tr key={idx}>
                ...
                <td className="px-6 py-4 whitespace-nowrap">
                {fund.sender.slice(0,15) ? 'complete' : 'not complete'}
                </td>
                <td className="px-6 py-4 whitespace-nowrap">
            ...
            rounded-full md:inline-flex" onClick={() =>{setGetModel(true); getData(idx); setGetProduct({
              ...getProduct,
