...
export default ({getModel, setGetModel, getFund,allShipmentsdata}) => {
  const [index , setIndex] = useState(0);
  const [singleShipmentData, setSingleShipmentData] = useState();

  const getShipmentData = async (ship) => {
    const getData = await getFund(ship);
    ...
    <div className="fixed inset-0 z-10 overflow-y-auto">
      <div className="fixed inset-0 w-full h-full bg-black opacity-40" onClick={()=> setGetModel(false)}>
        ...
        <div className="flex items-center min-h-screen px-4 py-8">
          ...
          <button onClick={()=> getShipmentData(shipment.index)} className='block w-full mt-3 py-4 font-medium text-sm text-center text-white bg-indigo-600
      ...
      {<tbody className="text-gray-600 divide-y">
        {allShipmentsdata?.map((shipment, idx) => (
          ...
          <tr key={idx}>
            ...
            rounded-full md:inline-flex" onClick={(shipment.index) => setGetModel(true)}>
             View
            </button>
