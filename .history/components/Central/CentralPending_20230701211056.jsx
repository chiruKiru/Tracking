...
export default ({getModel,setGetModel,alldata,RejectedFund,centralFund,getFundMain,currentUser}) => {
  ...
  const reject = () => {
    ...
    console.log(getProduct)
      if (result.value) {
        RejectedFund(getProduct,result.value)
      }
    });
  }
  ...
  const handleCopy = (text) => {
    navigator.clipboard.writeText(text)
      .then(() => {
        ...
        // Reset the "Copied" message and color after a certain time
        setTimeout(() => {
          setCopied(false);
        }, 2000);
      })
        ...
        {alldata?.map((fund, idx) => (
          fund.To == "Central"  && fund.status < 2 ?(
              <tr key={idx}>
                ...
                {fund.sender.slice(0,5)}...{fund.sender.slice(35,40)}
                </td>
                <td className="px-6 py-4 whitespace-nowrap">
                  {fund.receiver.slice(0,5)}...{fund.receiver.slice(35,40)}{"  "}   
                  <button
                  onClick={() => handleCopy(fund.receiver)}
                  className={copied ? 'copied' : ''}
                >
                  <FontAwesomeIcon
                icon={faCopy}
                className={copied ? 'copied-icon' : ''}
