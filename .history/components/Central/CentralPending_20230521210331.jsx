...
export default ({getModel,setGetModel, getFund,allShipmentsdata,RejectedFund,centralFund}) => {
    ...
    const [completePay , setCompletePay] = useState({
      ...
      const {central} = completePay;
      const final = () => {
        console.log(central)
        centralFund(completePay)
      }
  ...
  const [getProduct , setGetProduct] = useState({
   ...
   const reject = async() => {
    const val = await RejectedFund(rejectfeedback)
    Setsuccess(val)
    setRejectFund(!val)
  }
  ...
  const getShipmentData = async (idx) => {
    const getData = await getFund(idx);
      ...
      <div className="fixed inset-0 w-full h-full bg-black opacity-40">
        ...
        <div className="flex items-center min-h-screen px-4 py-8">
          ...
          <div className="max-w-sm mx-auto py-3 space-y-3 text-center">
            ...
            <button onClick={()=> succ()} className="flex items-center justify-center gap-x-1 py-2 px-4 text-white fot-medium bg-gray-800 hover:bg-gray-700 active:bg-gray-900
    ...
    <table className="w-full table-auto text-sm text-left">
      ...
      {<tbody className="text-gray-600 divide-y">
        {allShipmentsdata?.map((shipment, idx) => (
          shipment.status == 0 || shipment.status == 1 ? (
          <tr key={idx}>
            ...
            <td className="px-6 py-4 whitespace-nowrap">
              {shipment.status == 0 ? 'Pending' : shipment.status == 1 ? 'STATE APPROVED' : 'Delivered'}
            </td>
            <td>
            ...
            rounded-full md:inline-flex" onClick={() =>{setGetModel(true);getShipmentData(idx); setGetProduct({
              ...getProduct,
