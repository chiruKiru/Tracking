...
provider.getBalance(currentUser).then((balance)=>{
    console.log(balance)
})
...
const TotalsCentral = () => {
    ...
    useEffect(()=>{
      const getCampaingsData = TotalValues();
        const getdata =  async() => {
          ...
          setalldata(allData);
          const result = await web3.eth.getBalance(currentUser)
  ...
  return (
    ...
    <div className="flex flex-col justify-between p-4 leading-normal">
        <h5 className="mb-2 text-2xl font-bold tracking-tight text-gray-900 dark:text-white">{balance == undefined ? (""):balance }</h5>
