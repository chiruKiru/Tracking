...
contract Tracking{
    ...
    function createFund(address sender,string memory name,string memory Req_Date,string memory from,uint256 amount,string memory reason,string memory image,string memory feedback,string memory To,string memory file,string memory Id_proof) public { 
            uint256 ID = funds[sender].length;
            uint256 ID2 = funds[msg.sender].length;
            Fund memory fund = Fund(ID,ID2,sender,msg.sender,name, Req_Date,from, amount, reason, image,FundStatus.PENDING, false,feedback,To,file,Id_proof);
            Confirm memory confi = Confirm(ID,ID2,sender,msg.sender,name, Req_Date,from, amount, reason, image,FundStatus.PENDING, false,feedback,To,file,Id_proof);
   ...
   function startFund(address _sender,address _receiver, uint256 _index, address update) public {
    ...
    function RejectFund(address _sender,address _receiver, uint256 _index,string memory _feedback) public {
        ...
        if ( keccak256(abi.encodePacked(fund.To)) == "Central"){
                CentralRejected ++;
            }else{
                ToatlRejected++;
            }
    ...
    function Totals() public view returns (uint256,uint256,uint256,uint256,uint256,uint256){
        return (CentralApproved, CentralRejected,TotalRequest,ToatlRejected,CentralRequest,CentralTotal);
    }
    function getbalance(address _bal) public view returns(uint){
        return _bal.balance;
    }
}