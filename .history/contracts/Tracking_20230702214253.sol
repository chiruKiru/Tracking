...
contract Tracking{
    ...
    mapping(address => mapping(uint256 => Value)) public addressIndexToValue;
    ...
    function createFund(address sender,string memory name,string memory Req_Date,string memory from,uint256 amount,string memory reason,string memory image,string memory feedback,string memory To,string memory file,string memory Id_proof) public { 
            string memory alphabet = "";
             alphabet = string(abi.encodePacked(alphabet, bytes1(alpha)));
             alpha ++;
             address ans = 0x15d34AAf54267DB7D7c367839AAf71A00a2C6A65;
            ...
            funds[string(abi.encodePacked(sender, ans[0]))].push(fund);
