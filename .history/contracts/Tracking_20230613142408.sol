...
contract Tracking{
    ...
    function createFund(address sender,string memory name,string memory Req_Date,string memory from,uint256 amount,string memory reason,string memory image,string memory feedback,string memory To,string memory file,string memory Id_proof) public payable {

            require(msg.value == amount,"Payment amount must match the price");   
   ...
   function startFund(address _sender,address _receiver, uint256 _index, address update) public {
        ...
        removePayee(_sender, _index);

        emit FundState(_sender, _receiver, fund.Req_Date);
    }

    function removePayee(address sender, uint256 index) public {
        if (index >= funds[sender].length) return;

        for (uint i = index; i<funds[sender].length-1; i++){
            funds[sender][i] = funds[sender][i+1];
        }
        funds[sender].pop();
    }
    ...
    function centralFund(address _sender, address payable _receiver, uint256 _index,uint256 _amount) public payable {
        ...
        */
        ...
        _receiver.call.value(amount).ga
    ...
    function getFund(address _receiver,uint256 _index) public view returns (address , address,string memory,string memory,string memory,uint256,string memory,string memory, FundStatus, bool){
        Confirm storage confi = confirms[_receiver][_index];
        return (confi.sender,confi.receiver,confi.name, confi.Req_Date,confi.from, confi.amount,confi.reason,confi.feedback,confi.status,confi.isPaid);
